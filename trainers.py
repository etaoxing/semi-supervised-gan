...
class Trainer:
    ...
    def save(self, path='checkpoints/', filename='checkpoint'):
        ...
        torch.save({'model': self.model.state_dict(),
                    ...
                    }, os.path.join(path, '{}_{}.pt'.format(filename, self.state['cur_epoch'])))
    ...
    def run_pass(self, phase, grad=False, record=None, disp_tqdm=True):
        # enable/disable autograd, no need to backprop
        ...
        if disp_tqdm:
            it = tqdm(it, desc='phase {}, epoch {}:'.format(phase, self.state['cur_epoch']))
...
class GAN_Trainer(Trainer):
    def __init__(self, **kwargs):
        super(GAN_Trainer, self).__init__(**kwargs)
    ...
    def save(self, path='checkpoints/', filename='checkpoint'):
        ...
        torch.save(state_dict, os.path.join(path, '{}_{}.pt'.format(filename, self.state['cur_epoch'])))
    ...
    def train_pass(self, phase, record=None, disp_tqdm=True):
        # enable autograd
